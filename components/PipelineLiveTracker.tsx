...
import { INBOUND_LISTINGS, isPipelineCandidate, marketForState, type Listing } from '../lib/data/inboundListings'

// Live Deal Pipeline tracker — portal-managed active acquisitions (Sourcing → Closed), backed by
// /api/deal-pipeline. This is the editable side of the Deal Pipeline section (the workbook mirror is
// read-only). It also hosts the inbound-listing import: fit listings can be auto-added to the
// pipeline via /api/deal-pipeline/import-listings.
...
export default function PipelineLiveTracker() {
  ...
  const importListings = async (ids: string[], key: string) => {
    ...
    setImporting(key); setImportMsg('')
    try {
      const r = await fetch('/api/deal-pipeline/import-listings', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids }),
      })
      const d = await r.json()
      if (!r.ok) { setImportMsg(d.error || 'Import failed'); return }
      setImportMsg(`Added ${d.added} to pipeline${d.skipped ? ` · ${d.skipped} already present` : ''}.`)
      await load()
  ...
  const kpi = (label: string, value: string, color?: string) => (
    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '14px 18px', flex: 1, minWidth: 130 }}>
      ...
      <div style={{ fontSize: 24, fontWeight: 700, color: color || NAVY, marginTop: 4 }}>{value}</div>
    </div>
  )
  ...
  return (
    <div>
      ...
      {/* Inbound listings → pipeline (auto-add mechanism) */}
      <div style={{ background: '#f8fbff', border: '1px solid #cfe0f5', borderRadius: 12, padding: 14, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: NAVY }}>📥 Auto-add from inbound listings</div>
            <div style={{ fontSize: 11.5, color: '#64748b', marginTop: 2, maxWidth: 620, lineHeight: 1.5 }}>
              Fit listings that clear Buy-Box triage and aren’t already in the pipeline. Adding creates a Sourcing-stage deal; the same endpoint backs an eventual auto-capture agent.
            </div>
          </div>
          <button
            onClick={() => importListings(candidates.map((l) => l.id), 'all')}
            disabled={!candidates.length || !!importing}
            style={{ padding: '8px 14px', borderRadius: 8, border: 'none', background: candidates.length ? '#0e7490' : '#cbd5e1', color: '#fff', cursor: candidates.length && !importing ? 'pointer' : 'default', fontSize: 12.5, fontWeight: 600, whiteSpace: 'nowrap' }}>
            {importing === 'all' ? 'Adding…' : `Auto-add all fit (${candidates.length})`}
          </button>
        </div>
        {importMsg && <div style={{ fontSize: 11.5, color: '#0e7490', marginTop: 8, fontWeight: 600 }}>{importMsg}</div>}
        {candidates.length > 0 ? (
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 12 }}>
            {candidates.map((l: Listing) => {
              const capPct = ((l.inPlaceNoi / l.askingPrice) * 100).toFixed(1)
              return (
                <div key={l.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '10px 12px', flex: '1 1 300px', minWidth: 280 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 12.5, fontWeight: 700, color: '#111827' }}>{l.address}</div>
                      <div style={{ fontSize: 11, color: '#64748b', marginTop: 1 }}>
                        {marketForState(l.state)} · {usd(l.askingPrice)} · {capPct}% cap · {l.source}
                      </div>
                    </div>
                    <button onClick={() => importListings([l.id], l.id)} disabled={!!importing}
                      style={{ flexShrink: 0, padding: '5px 11px', borderRadius: 7, border: `1px solid ${NAVY}`, background: '#fff', color: NAVY, cursor: importing ? 'default' : 'pointer', fontSize: 11.5, fontWeight: 600 }}>
                      {importing === l.id ? 'Adding…' : '+ Add'}
                    </button>
                  </div>
                </div>
              )
          ...
          <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 10 }}>
            {loading ? 'Checking listings…' : 'No new fit listings — all eligible inbound deals are already in the pipeline.'}
          </div>
        )}
      </div>
        ...
        {STAGES.map((s) => {
          ...
          return (
            <div key={s} style={{ flex: 1, minWidth: 92, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '8px 10px' }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: NAVY, marginTop: 3 }}>{n}</div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      {r.purchase_price != null && r.purchase_price > 0 && <span style={{ fontSize: 12, fontWeight: 600, color: NAVY }}>{usd(r.purchase_price)}</span>}
                    </div>
      ...
      {editing && (
        <div onClick={() => setEditing(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: 14, padding: 22, width: 560, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: NAVY, marginBottom: 16 }}>{editing.id ? 'Edit deal' : 'Add deal'}</div>
            {(() => {
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      ...
                      <button onClick={save} disabled={saving || !editing.deal_name?.trim()} style={{ border: 'none', background: NAVY, color: '#fff', borderRadius: 8, padding: '8px 18px', fontSize: 13, fontWeight: 600, cursor: saving ? 'default' : 'pointer', opacity: saving || !editing.deal_name?.trim() ? .6 : 1 }}>{saving ? 'Saving…' : 'Save'}</button>
                    </div>
                  </div>
                </div>
              )
