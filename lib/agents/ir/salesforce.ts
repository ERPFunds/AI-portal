...
/**
 * Read-only discovery for the Capital Raising pipeline: report the org's Opportunity stage funnel
 * (with default probabilities), record types, whether the standard pipeline fields exist, open-opp
 * counts by stage, and a few sample open opportunities. Writes nothing. Used to design the SF sync.
 */
export async function opportunityPipelineProbe(): Promise<Record<string, unknown>> {
  if (!salesforceConfigured()) return { configured: false };
  const out: Record<string, unknown> = { configured: true };

  // Full stage funnel + default probabilities (the org's real ordering).
  try {
    const rows = await sfQuery("SELECT MasterLabel, DefaultProbability, IsClosed, IsWon, SortOrder FROM OpportunityStage ORDER BY SortOrder");
    out.stages = rows.map((r) => ({ label: r.MasterLabel, probability: r.DefaultProbability, isClosed: r.IsClosed, isWon: r.IsWon }));
  } catch (e) { out.stagesErr = String(e).slice(0, 160); }

  // Record types (e.g. Fund IV vs DST/1031).
  try {
    const rt = await sfQuery("SELECT Name, DeveloperName FROM RecordType WHERE SObjectType = 'Opportunity' AND IsActive = true");
    out.recordTypes = rt.map((r) => r.Name);
  } catch (e) { out.recordTypesErr = String(e).slice(0, 160); }

  // Which standard pipeline fields are present.
  try {
    const f = await describeFieldsRaw("Opportunity");
    const names = new Set(f.map((x) => x.name));
    out.hasFields = Object.fromEntries(["Name", "Amount", "Probability", "StageName", "NextStep", "CloseDate", "OwnerId", "Type", "LastActivityDate", "ForecastCategoryName"].map((n) => [n, names.has(n)]));
  ...
  // Totals.
  try {
    const t = await sfQuery("SELECT COUNT(Id) cnt FROM Opportunity");
    const open = await sfQuery("SELECT COUNT(Id) cnt FROM Opportunity WHERE IsClosed = false");
    out.totalOpps = t[0]?.cnt ?? 0;
