...
export default function DashboardClient({ roleKey, userEmail, userName }: Props) {
  ...
  const views: Record<string, React.ReactNode> = {
    ...
    lp: <LpDirectoryView />,
    'crm-lp-directory': <InvestorCrmView program="PE" mode="lps" onNavigate={setCurrentView} />,
...
function LpDirectoryView() {
  ...
  async function composeSend() {
    ...
    setComposeBusy('send')
    try {
      const res = await fetch('/api/agent-inbox', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'send-compose', to: compose.to.trim(), from: compose.from, subject: compose.subject, body: compose.body, contactName: compose.lp.contact || '', lpName: compose.lp.investor || '' }) })
      const j = await res.json()
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          <input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') run() }}
            placeholder="Ask the knowledge base — a figure, a name, a term, an explanation…"
