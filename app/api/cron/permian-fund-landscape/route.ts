...
export async function GET(request: Request) {
  ...
  try {
    const [rawNews, seenUrls] = await Promise.all([
      fetchFundNews(),
      getSeenNewsletterArticleUrls('permian-fund-landscape').catch(() => new Set<string>()),
    ]);
    const news = rawNews.filter((item) => !seenUrls.has(item.link));

    if (news.length === 0) {
      logAgentRun({ agentId: "lp-intel", workflowId: "permian-fund-landscape", status: "error", market: "permian", durationMs: Date.now() - startMs, errorMessage: "No articles found — skipping send" }).catch(() => {});
  ...
  <tr><td align="center">
    <table width="100%" style="max-width:640px;background:#fff;border-radius:8px;overflow:hidden;">
      <tr><td style="background:#0f172a;padding:28px 32px;">
        ...
        <div style="font-size:13px;color:#cbd5e1;margin-top:6px;">Competitor activity &amp; fund benchmarks &middot; Permian Basin Industrial</div>
      </td></tr>
      ...
      <tr><td style="padding:18px 32px;background:#f8fafc;border-top:1px solid #e5e7eb;text-align:center;">
        <div style="font-size:12px;color:#9ca3af;">ERP Funds AI Portal &middot; Permian Fund Landscape Brief &middot; Weekly</div>
      </td></tr>
    </table>
  </td></tr>
</table>
