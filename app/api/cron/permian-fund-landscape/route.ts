...
import { generatePermianFundCompetitorBrief } from "@/lib/agents/workflows/permian-merged-briefs";
...
export async function GET(request: Request) {
  ...
  const seenUrls = await getSeenNewsletterArticleUrls("permian-fund-landscape").catch(() => new Set<string>());

  try {
    const startMs = Date.now();
    const { subject, htmlBody, summary, newsItems } = await generatePermianFundCompetitorBrief(period, { excludeUrls: seenUrls });
    const emailResult = await sendEmailViaGraph({ subject, htmlBody });
    await saveNewsletterToSharePoint({ market: "Permian", briefType: "Fund Landscape", htmlBody }).catch(() => {});
    await archiveBrief({ agentName: "permian-fund-landscape", subject, html: htmlBody, narrative: summary, macro: {}, news: newsItems }).catch(() => {});
    await logAgentRun({ agentId: "lp-intel", workflowId: "permian-fund-landscape", status: emailResult.success ? "success" : "error", summary, market: "permian", durationMs: Date.now() - startMs, errorMessage: emailResult.success ? undefined : emailResult.message }).catch(() => {});
    return NextResponse.json({ success: emailResult.success, period, articles: newsItems.length, subject });
  } catch (err) {
    console.error("[permian-fund-landscape] failed:", err);
    await logAgentRun({ agentId: "lp-intel", workflowId: "permian-fund-landscape", status: "error", market: "permian", errorMessage: String(err) }).catch(() => {});
