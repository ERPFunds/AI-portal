...
import { generatePermianSubmarketBrief } from "@/lib/agents/workflows/permian-merged-briefs";
...
export async function GET(request: Request) {
  ...
  const seenUrls = await getSeenNewsletterArticleUrls("permian-submarket-watch").catch(() => new Set<string>());

  try {
    const startMs = Date.now();
    const { subject, htmlBody, summary, newsItems } = await generatePermianSubmarketBrief(period, { excludeUrls: seenUrls });
    const emailResult = await sendEmailViaGraph({ subject, htmlBody });
    await saveNewsletterToSharePoint({ market: "Permian", briefType: "Submarket Watch", htmlBody }).catch(() => {});
    await archiveBrief({ agentName: "permian-submarket-watch", subject, html: htmlBody, narrative: summary, macro: {}, news: newsItems }).catch(() => {});
    await logAgentRun({ agentId: "lp-intel", workflowId: "permian-submarket-watch", status: emailResult.success ? "success" : "error", summary, market: "permian", durationMs: Date.now() - startMs, errorMessage: emailResult.success ? undefined : emailResult.message }).catch(() => {});
    return NextResponse.json({ success: emailResult.success, period, articles: newsItems.length, subject });
  } catch (err) {
    console.error("[permian-submarket-watch] failed:", err);
    await logAgentRun({ agentId: "lp-intel", workflowId: "permian-submarket-watch", status: "error", market: "permian", errorMessage: String(err) }).catch(() => {});
