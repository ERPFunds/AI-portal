...
async function runBackfill(req: NextRequest, months: number, dryRun: boolean, mailboxes: string, max: number, reimport: boolean) {
  ...
  const results = await Promise.all(
    boxes.map(async (mb) => {
      const url =
        ...
        `?force=1&sinceMonths=${months}&max=${max}&dryRun=${dryRun ? 1 : 0}&reimport=${reimport ? 1 : 0}&mailbox=${encodeURIComponent(mb)}`;
      try {
...
export async function GET(req: NextRequest) {
  ...
  const reimport = p.get("reimport") === "1";
  return NextResponse.json(await runBackfill(req, months, dryRun, mailboxes, max, reimport));
}

// POST — run for real (files into the IR subfolders). Body: { months, dryRun, mailboxes, max }
export async function POST(req: NextRequest) {
  if (!(await requireUser())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  let body: { mailboxes?: string; months?: number; dryRun?: boolean; max?: number; reimport?: boolean } = {};
  ...
  const reimport = body.reimport ?? false;
  return NextResponse.json(await runBackfill(req, months, dryRun, mailboxes, max, reimport));
}
