  ...
  try {
    ...
    const msg = await anthropic.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `You are an industrial CRE market analyst for ERP Industrials. Write a Submarket Watch brief (4-5 paragraphs) covering the following news focused on the Florida industrial market — particularly Tampa, Brevard County, and the Space Coast.

Focus on:
1. Sale comparable transactions — what are assets trading at? Cap rates, price/SF, price/acre?
2. Tenant activity — who's leasing, expanding, contracting in Florida industrial markets?
3. Submarket trends — vacancy, absorption, asking rents, any notable market shifts
4. OM implications — what does this week's activity mean for ERP's active Florida deals?
  ...
  <tr><td align="center">
    <table width="100%" style="max-width:640px;background:#fff;border-radius:8px;overflow:hidden;">
      <tr><td style="background:#0f172a;padding:28px 32px;">
        ...
        <div style="font-size:13px;color:#cbd5e1;margin-top:6px;">Sale comps &amp; tenant activity · Florida Industrial</div>
      </td></tr>
      ...
      <tr><td style="padding:18px 32px;background:#f8fafc;border-top:1px solid #e5e7eb;text-align:center;">
        <div style="font-size:12px;color:#9ca3af;">ERP Funds AI Portal · Brevard Submarket Watch · Weekly</div>
      </td></tr>
    </table>
  </td></tr>
</table>
    ...
    await archiveBrief({ agentName: "brevard-submarket-watch", subject, html, narrative, macro: {}, news });
    await sendBriefEmail({ subject, html });
    saveNewsletterToSharePoint({ market: "Brevard", briefType: "Submarket Watch", htmlBody: html }).catch(() => {});
    logAgentRun({ agentId: "lp-intel", workflowId: "brevard-submarket-watch", status: "success", summary: narrative.slice(0, 300), market: "brevard", durationMs: Date.now() - startMs }).catch(() => {});
