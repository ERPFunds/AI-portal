...
// Parse a response body without throwing on an empty/non-JSON body (e.g. an auth redirect to the
// login page on a preview domain returns HTML, which would otherwise blow up res.json()).
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function safeJson(res: Response): Promise<any> {
  let text = ''
  try { text = await res.text() } catch { return {} }
  if (!text.trim()) return {}
  try { return JSON.parse(text) } catch { return { error: res.status === 401 || /<!doctype|<html/i.test(text) ? 'Sign in required (open the app from the same domain you logged in on)' : text.slice(0, 200) } }
}
...
export default function TenantCrmView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!tRes.ok || tJson.error) { setError(String(tJson.error ?? `Load failed (${tRes.status})`)); return }
      setTenants((tJson.tenants as Tenant[]) ?? [])
      setProperties((tJson.properties as PropertyOpt[]) ?? [])
      const pJson = await safeJson(pRes)
      setProspects((pJson.items as Prospect[]) ?? [])
      setError(null)
  ...
  useEffect(() => {
    ...
    async function counts() {
      const entries = await Promise.all(tenants.map(async t => {
        try { const r = await fetch(`/api/tenant-crm/contacts?tenant_id=${t.id}`); const j = await safeJson(r); return [t.id, (j.contacts ?? []).length] as const }
...
function TenantDrawer({ tenant, properties, onClose, onEdit, onDelete, onStatus, onContactsChanged }: {
  ...
  const loadContacts = useCallback(async () => {
    try { const r = await fetch(`/api/tenant-crm/contacts?tenant_id=${tenant.id}`); const j = await safeJson(r); setContacts(j.contacts ?? []); onContactsChanged((j.contacts ?? []).length) }
    catch { setContacts([]) }
