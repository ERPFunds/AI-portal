...
// Property lenders directory. Portal-managed, team-wide. RLS-locked → service-role client.
...
const COLS = "id, name, lender_type, contact, email, phone, status, products, markets, notes, created_by, created_at, updated_by, updated_at";

function clean(body: Record<string, unknown>) {
  const out: Record<string, unknown> = {};
  if (typeof body.name === "string") out.name = body.name.trim();
  if (body.lender_type != null) out.lender_type = TYPES.includes(String(body.lender_type)) ? String(body.lender_type) : "Other";
  ...
  if (body.products != null) out.products = String(body.products).trim() || null;
...
export async function GET() {
  ...
  const { data, error } = await supabase.from("lenders").select(COLS).order("name", { ascending: true });
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase.from("lenders").insert({ ...row, created_by: by, updated_by: by }).select(COLS).single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PATCH(req: NextRequest) {
  ...
  const { data, error } = await supabase.from("lenders").update({ ...row, updated_by: user.email ?? user.id, updated_at: new Date().toISOString() }).eq("id", id).select(COLS).single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ item: data });
}
